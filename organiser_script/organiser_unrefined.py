	...
	if not os.path.isdir(path):
		...
		for index in range(len(operable_list)):
			...
			if not os.path.isdir(item):
				...
				move_to_function(item, formats)


def copy_to_function(path, formats):
	if not os.path.isdir(path):
		path_split_list = path.split('.')
		file_extension_index = len(path_split_list) - 1
		file_extension_name = path_split_list[file_extension_index]
		for extension in formats:
			if extension == file_extension_name:
				try:
					path_split_list = path.split('\\')
					file_name_index = len(path_split_list) - 1
					file_name = path_split_list[file_name_index]
					...
					log.write(
						f'copied {file_name} successfully from {path} to {destinations[keys[i]]} during {time.asctime()} \n')
					break
				except OSError:
					print(f'error occurred while copying {file_name} from {path}')
					log.write(f'error occurred while copying {file_name} from {path} during {time.asctime()} \n')
					break
	elif os.path.isdir(path):
		operable_list = os.listdir(path)
		for index in range(len(operable_list)):
				...
				for extension in formats:
					if extension == item_extension_name:
						try:
							...
							print(f'error occurred while copying {operable_list[index]} from {item}')
							log.write(
								f'error occurred while copying {operable_list[index]} from {item} during {time.asctime()} \n')
							break
			elif os.path.isdir(item):
				print(f'performing operation on {item} with {format_names[i]}')
				log.write(f'performing operation on{item} with {format_names[i]} at {time.asctime()} \n')
...
if __name__ == '__main__':
	print('loading....')
	time.sleep(1)
	print('want to use "organise" or a simple "bulk-paste"')
	time.sleep(1)
	print(
	...
	if user_input.lower() == 'o':
		...
		for i in range(len(format_names)):
			for route in paths_to_operate:
				...
				if user_decision.lower() == 'm':
					move_to_function(route, format_list[i])
				elif user_decision.lower() == 'c':
					copy_to_function(route, format_list[i])
		...
		for i in range(len(format_names)):
			for path in source_paths_list:
				if user_inpt.lower() == 'c':
					copy_to_function(path, format_list[i])
				if user_inpt.lower() == 'm':
					move_to_function(path, format_list[i])
